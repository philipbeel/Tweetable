...
(function($){$.fn.tweetable=function(options){var defaults={limit:5,username:'philipbeel',retweets:false,time:false,replies:false,position:'append'};var options=$.extend(defaults,options);return this.each(function(options){var act=$(this);var $tweetList;var api="http://api.twitter.com/1/statuses/user_timeline.json?screen_name=";var count="&count=";var rts="&include_rts=";$.getJSON(api+defaults.username+count+defaults.limit+rts+defaults.retweets+"&callback=?",act,function(data){$.each(data,function(i,item){if(i==0){$tweetList=$('<ul class="tweetList">')[defaults.position.toLowerCase()+'To'](act);}
